...
## Purpose
Enable keyboard-driven interaction with the OpenSpec status widget. The user SHALL be able to summon an interactive overlay, navigate active changes with arrow keys, and trigger OpenSpec prompt templates with single letter keys — all without typing commands manually.
...
- **THEN** the preview pane shows the change name, schema name, artifact statuses with full names and colored icons, and a task progress bar
...
The user SHALL trigger OpenSpec prompt templates by pressing a single letter key while the overlay is open.
...
The overlay SHALL display a hint bar showing available action keys and their functions.

#### Scenario: Full hint bar with changes
- **WHEN** the overlay is open and active changes exist
